...
describe('DynamicApiModule forRoot (e2e)', () => {
  ...
  describe('Websockets', () => {
    ...
    describe('Authentication EVENTS', () => {
      ...
      //     it('should throw a bad request exception if validation fails', async () => {
      //       const { body, status } = await server.post('/auth/register', { email: 'unit.test.co', password: 'test-2'
      // });  expect(status).toBe(400); expect(body).toEqual({ error: 'Bad Request', message: [ 'email must be an
      // email', 'password is not strong enough', ], statusCode: 400, }); });  it('should create a new user and return
      // access token if the validation was successful', async () => { const { body, status } = await
      // server.post('/auth/register', { email: 'unit@test.co', password: 'Test-2' });  expect(status).toBe(201);
      // expect(body).toEqual({ accessToken: expect.any(String) }); }); });  describe('POST /auth/login', () => {
      // beforeEach(async () => { await server.post('/auth/register', { email: 'unit@test.co', password: 'Test-2' });
      // });  it('should throw an unauthorized exception if payload contains non whitelisted property', async () => {
      // const { body, status } = await server.post( '/auth/login', { email: 'unit@test.co', password: 'Test-2', role:
      // 'ADMIN' }, );  expect(status).toBe(400); expect(body).toEqual({ error: 'Bad Request', message: ['property role
      // should not exist'], statusCode: 400, }); });  it('should throw an unauthorized exception if email is missing',
      // async () => { const { body, status } = await server.post('/auth/login', { password: 'Test-2' });
      // expect(status).toBe(401); expect(body).toEqual({ message: 'Unauthorized', statusCode: 401, }); });  it('should
      // throw an unauthorized exception if password is missing', async () => { const { body, status } = await
      // server.post('/auth/login', { email: 'unit@test.co' });  expect(status).toBe(401); expect(body).toEqual({
      // message: 'Unauthorized', statusCode: 401, }); });  it('should return access token if the validation was
      // successful', async () => { await server.post('/auth/register', { email: 'unit@test.co', password: 'test' });
      // const { body, status } = await server.post('/auth/login', { email: 'unit@test.co', password: 'Test-2' });
      // expect(status).toBe(200); expect(body).toEqual({ accessToken: expect.any(String) }); }); }); });
      ...
      //     it('should throw an unauthorized exception if user is not logged in and protected is true', async () => {
      //       const { body, status } = await server.post('/auth/register', { email: 'unit@test.co', password: 'test'
      // });  expect(status).toBe(401); expect(body).toEqual({ message: 'Unauthorized', statusCode: 401, }); }); });
      // describe('abilityPredicate', () => { it('should not create a new user if user is not verified', async () => {
      // const { email, password } = user; const { body: { accessToken } } = await server.post('/auth/login', { email,
      // password });  const { body, status } = await server.post('/auth/register', { email: 'unit@test.co', password:
      // 'test' }, { headers: { Authorization: `Bearer ${accessToken}` }, });  expect(status).toBe(403);
      // expect(body).toEqual({ error: 'Forbidden', message: 'Access denied', statusCode: 403, }); });  it('should
      // create a new user and return access token if user is verified', async () => { const { email, password } =
      // admin; const { body: { accessToken } } = await server.post('/auth/login', { email, password });  const { body,
      // status } = await server.post('/auth/register', { email: 'unit@test.co', password: 'test' }, { headers: {
      // Authorization: `Bearer ${accessToken}` }, });  expect(status).toBe(201); expect(body).toEqual({ accessToken:
      // expect.any(String) }); }); });  describe('additionalFields', () => { it('should allow to register a new user
      // with additional fields', async () => { const { email, password } = admin; const { body: { accessToken } } =
      // await server.post('/auth/login', { email, password });  const { body, status } = await server.post(
      // '/auth/register', { email: 'client@test.co', password: 'client', role: 'client' }, { headers: { Authorization:
      // `Bearer ${accessToken}` }, }, );  expect(status).toBe(201); expect(body).toEqual({ accessToken:
      // expect.any(String) }); }); });  describe('callback', () => { it('should not set isVerified to true if role is
      // not admin', async () => { const { email, password } = admin; const { body: loginBody } = await
      // server.post('/auth/login', { email, password });  const { body: { accessToken } } = await server.post(
      // '/auth/register', { email: 'client@test.co', password: 'client', role: 'client' }, { headers: { Authorization:
      // `Bearer ${loginBody.accessToken}` }, }, );  const { body, status } = await server.get( '/auth/account', {
      // headers: { Authorization: `Bearer ${accessToken}` } }, );  expect(status).toBe(200);
      // expect(body).toHaveProperty('isVerified', false); });  it('should set isVerified to true if role is admin',
      // async () => { const { email, password } = admin; const { body: loginBody } = await server.post('/auth/login',
      // { email, password });  const { body: { accessToken } } = await server.post( '/auth/register', { email:
      // 'admin2@test.co', password: 'admin2', role: 'admin' }, { headers: { Authorization: `Bearer
      // ${loginBody.accessToken}` }, }, );  const { body, status } = await server.get( '/auth/account', { headers: {
      // Authorization: `Bearer ${accessToken}` } }, );  expect(status).toBe(200);
      // expect(body).toHaveProperty('isVerified', true); }); }); });
