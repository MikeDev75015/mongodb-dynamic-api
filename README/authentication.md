...
> **⚠️ Application-wide scope:** as soon as `useAuth` is configured, `DynamicApiJwtAuthGuard` is registered as a **global** guard (`APP_GUARD`). It protects **every** controller in your Nest application, not just the routes generated by DynamicAPI — including hand-written controllers you add yourself. Decorate a handler or controller with `@Public()` (from `mongodb-dynamic-api`) to opt it out. Getting an unexpected 401 on a route you didn't expect to be guarded is almost always this. See [Debugging → `MONGODB_DYNAMIC_API_LOGGER`](./debugging.md#mongodb_dynamic_api_logger) to see the real rejection reason (missing/invalid/expired token, bad signature) server-side — it's never included in the client-facing 401 response.
